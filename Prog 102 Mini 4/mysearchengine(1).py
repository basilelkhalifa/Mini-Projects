...
class crawler:
    ...
    def addtoindex(self, url, soup, depth):
        ...
        print 'Indexing ' + url + ' [depth: %d]' % depth
        url = smart_str(url)
    ...
    def crawl(self, pages, depth=2):
        for i in range(depth):
            ...
            for page in pages:
                ...
                added = self.addtoindex(page, soup, i)
...
class searcher:
    ...
    def pagerankscore(self, results):
        self.pagerank = shelve.open(self.dbtables['pagerank'], flag='r')
        self.pageranks = dict([(url, self.pagerank.get(url, 0.15)) for url in results])
    ...
    def opendb(self):
        # {url:outgoing_link_count}
        self.urllist = shelve.open(self.dbtables['urllist'], flag='c')
        #{word:{url:[loc1, loc2, ..., locN]}}
        self.wordlocation = shelve.open(self.dbtables['wordlocation'], flag='c')
        #{tourl:{fromUrl:None}}
        self.link = shelve.open(self.dbtables['link'], flag='c')
        #{word:[(urlFrom, urlTo), (urlFrom, urlTo), ..., (urlFrom, urlTo)]}
        self.linkwords = shelve.open(self.dbtables['linkwords'], flag='c')
